...
import { CreateBusinessDTO, EditBusinessDTO } from "@seekNseat/contracts/business";
...
export const eventTransformers = {
    BusinessWasCreated: (event: Event<CreateBusinessDTO>) =>
        new BusinessWasCreated(event.aggregateId, event.payload.name, event.payload.contactPhone),
    BusinessProfileWasEdited: (event: Event<EditBusinessDTO>) =>
        new BusinessProfileWasEdited(event.aggregateId, event.payload.name, event.payload.contactPhone, event.payload.address, event.payload.description),
    BusinessWasDeleted: (event: Event) => new BusinessWasDeleted(event.aggregateId)
}